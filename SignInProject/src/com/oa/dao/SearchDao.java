	...
	public static ArrayList<ProductItem> searchKeyword(String keyword){
		...
		try{
			...
			while(rs.next() && i<size){
				...
				/*
				// highestPrice
				pst2 = conn.prepareStatement("select i.itemname, a.bid_end_time, b.bid_price "
						+ "from items i,auctions a, bids b where b.auctions_fk = a.id "
						+ "AND a.items_fk = i.id AND i.id = " 
						+ productItem.getProductId()+ "  order by  b.bid_price DESC Limit 1");
				...
				pst2 = conn.prepareStatement("select i.itemname, a.bid_end_time, b.bid_price "
						+ "from items i,auctions a, bids b where b.auctions_fk = a.id "
						+ "AND a.items_fk = i.id AND i.id = " 
						+ productItem.getProductId()+ "  order by  b.bid_price ASC Limit 1");
				rs2 = pst2.executeQuery();
				if(rs2.next()) {
					productItem.setLowestPrice(rs2.getString("bid_price"));	
				}
				else {
					productItem.setLowestPrice("99999.00");			
				}
				// Auction
			...
			public static String getHighestPriceByProductItemID(String productitemid){
				...
				try{
					pst = conn.prepareStatement("select i.itemname, a.bid_end_time, b.bid_price "
							+ "from items i,auctions a, bids b where b.auctions_fk = a.id "
							+ "AND a.items_fk = i.id AND i.id = " 
							+ productitemid + "  order by  b.bid_price DESC Limit 1");
					rs = pst.executeQuery();
					if(rs.next()) {
			...
			public static String getLowestPriceByProductItemID(String productitemid){
				...
				try{
					pst = conn.prepareStatement("select i.itemname, a.bid_end_time, b.bid_price "
							+ "from items i,auctions a, bids b where b.auctions_fk = a.id "
							+ "AND a.items_fk = i.id AND i.id = " 
							+ productitemid + "  order by  b.bid_price ASC Limit 1");
					rs = pst.executeQuery();
					if(rs.next()) {
						 return rs.getString("bid_price");	
					}
...
////
		...
		public static Auction getAuction(String productitemid){
			...
			//Todo: need to check auction table first
			try{
				pst = conn.prepareStatement("select * from auction where items_fk=?");
				pst.setString(1, productitemid);
